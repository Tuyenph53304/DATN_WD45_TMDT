...
@section('title', 'Tạo Voucher mới - Admin Panel')
@section('page-title', 'Tạo Voucher mới')

@section('content')
<div class="card">
  <div class="card-header">
    <h3 class="card-title"><i class="bi bi-plus-circle me-2"></i> Tạo Voucher mới</h3>
  ...
  <div class="card-body">
    <form action="{{ route('admin.vouchers.store') }}" method="POST">
        ...
        <div class="col-md-4">
          <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control @error('min_order') is-invalid @enderror"
                   id="min_order" name="min_order" value="{{ old('min_order', 0) }}" min="0">
            @error('min_order')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
      ...
      <div class="row">
        <div class="col-md-4">
          <div class="mb-3">
            <label for="max_discount" class="form-label">Giảm tối đa (₫) - Chỉ áp dụng cho phần trăm</label>
            <input type="number" step="0.01" class="form-control @error('max_discount') is-invalid @enderror"
                   id="max_discount" name="max_discount" value="{{ old('max_discount') }}" min="0">
            @error('max_discount')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
